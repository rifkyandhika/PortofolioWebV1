	...
	$("#start").velocity("transition.whirlOut", {
		...
		complete: function () {
			$("#text").velocity({
				...
				complete: function () {
					callMenu();
					$("#menu ul li a[href='introduction']").trigger("click");
...
function introduction() {
	$("#introduction img").velocity("transition.flipYIn", { duration: 1500 });
	$("#introduction .title").velocity("transition.slideUpIn", { duration: 1500 });
	$("#introduction div").velocity("transition.slideDownIn", { duration: 1500 });
}

function my_portofolio() {
